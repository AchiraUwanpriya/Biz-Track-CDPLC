...
// // UPDATED: Removed 'range' from VIEWS array
// const VIEWS = [
//   { key: "week",  label: "Week"  },
//   { key: "month", label: "Month" },
//   { key: "year",  label: "Year"  },
//   // 'range' removed from here
// ];
...
// const WeekAxisTick = ({ x, y, payload, index, weekData }) => {
//   const item = weekData?.[index];
//   if (!item) return null;
//   return (
//     <g transform={`translate(${x},${y})`}>
//       <text x={0} y={0} dy={13} textAnchor="middle" fontSize={10} fill="#64748b">
//         {payload.value}
//       </text>
//       {item.showMonthLabel && (
//         <text x={0} y={0} dy={28} textAnchor="middle" fontSize={10} fontWeight={700} fill="#1a2d4d">
//           {item.monthDisplay}
//         </text>
//       )}
//     </g>
...
//   const labelHeight = isWeekView ? 42 : 30;
...
// // info so the x-axis can show "1,2,3,4" bars grouped under a month label.
// const enrichWeekDataWithMonth = (weeks) => {
//   if (!weeks || weeks.length === 0) return weeks;

//   // 1) figure out each week's position-in-month (resets when the month changes)
//   const monthCounter = {};
...
//     return {
//       ...w,
//       name: String(monthCounter[monthKey]),   // bar label becomes "1","2","3"...
//       monthKey,
//       monthLabel: d.toLocaleDateString('en-US', { month: 'short' }),
//     };
//   });

//   //    middle week of each group to carry the visible month label
//   const groups = [];
//   enriched.forEach((w, idx) => {
//     const last = groups[groups.length - 1];
//     if (last && last.monthKey === w.monthKey) {
//       last.indices.push(idx);
//     } else {
//       groups.push({ monthKey: w.monthKey, monthLabel: w.monthLabel, indices: [idx] });
//     }
//   });
//   groups.forEach((g) => {
//     const midIdx = g.indices[Math.floor((g.indices.length - 1) / 2)];
//     enriched[midIdx].showMonthLabel = true;
//     enriched[midIdx].monthDisplay = g.monthLabel;
//   });
...
//     // ── WEEK VIEW: one bar per calendar week, averaged (~52 bars/year) ──

//     const rawWeekData = aggregateByPeriod(
//       weeklyApiData,
//       (date) => getWeekStart(date).toISOString().slice(0, 10),
//       (date) => getWeekStart(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
//       (date) => getWeekStart(date).getTime()
//     );
//     const weekData = enrichWeekDataWithMonth(rawWeekData);

//     // ── MONTH VIEW: one bar per calendar month, averaged (~12 bars/year) ──
...
//     // ── YEAR VIEW: one bar per calendar year, averaged ──
//     const yearData = aggregateByPeriod(
//       weeklyApiData,
//       (date) => `${date.getFullYear()}`,
//       (date) => `${date.getFullYear()}`,
//       (date) => new Date(date.getFullYear(), 0, 1).getTime()
//     );
...
// // ─── UPDATED: WeekAxisTick with ALL Week Numbers ──────────────────────────
// const WeekAxisTick = ({ x, y, payload, index, weekData }) => {
//   const item = weekData?.[index];
//   if (!item) return null;
...
// // ─── UPDATED: enrichWeekDataWithMonth - Shows ALL Weeks (1-53) ────────────
// const enrichWeekDataWithMonth = (weeks) => {
//   if (!weeks || weeks.length === 0) return weeks;

//   // Calculate week of year for each entry
//   const enriched = weeks.map((w) => {
//     const d = new Date(w.fullDate);
...
//     return {
//       ...w,
//       name: String(weekNumber),  // Bar label becomes "1", "2", "3", ... "53"
//       weekNumber: weekNumber,
//       year: year,
//       monthLabel: monthName,
//       fullLabel: `Week ${weekNumber} (${monthName} ${year})`,
//     };
//   });

//   // Group by month for month label positioning
//   const monthGroups = [];
//   let currentMonth = null;
  
//   enriched.forEach((w, idx) => {
//     const monthKey = `${w.year}-${w.monthLabel}`;
...
//     } else {
//       currentMonth.indices.push(idx);
//       currentMonth.weekNumbers.push(w.weekNumber);
//     }
//   });
  
//   // Add month labels to middle of each month group
//   monthGroups.forEach((g) => {
//     if (g.indices.length > 0) {
//       const midIdx = g.indices[Math.floor((g.indices.length - 1) / 2)];
//       enriched[midIdx].showMonthLabel = true;
//       // Show month name and year
//       enriched[midIdx].monthDisplay = `${g.monthLabel} ${g.year}`;
//     }
//   });
...
//         // Use year + week number as key for proper ordering
//         return `${year}-${String(weekNum).padStart(2, '0')}`;
//       },
//       (date) => {
//         const weekStart = getWeekStart(date);
//         const weekNum = getWeekNumber(weekStart);
//         return String(weekNum); // Just the week number
//       },
//       (date) => {
//         const weekStart = getWeekStart(date);
//         const weekNum = getWeekNumber(weekStart);
